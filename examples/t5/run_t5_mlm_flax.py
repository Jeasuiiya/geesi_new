...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        data_obj = load_from_disk("./norwegian-t5-base")
        ...
        datasets = DatasetDict({"train": data_train, "validation": data_val})
    ...
    def train_step(state, batch, dropout_rng):
        # dropout_rng, new_dropout_rng = jax.random.split(dropout_rng) #修改1 —— 注释掉整行
        ...
        grad_fn = method.value_and_grad(loss_fn)# 修改2 —— 把jax的方法改成method的方法
    ...
    for epoch in epochs:
        ...
        for step, batch_idx in enumerate(tqdm(train_batch_idx, desc="Training...", position=1)):
            ...
            dropout_rngs, new_dropout_rng = jax.random.split(dropout_rngs) # 修改6 —— 对dropout_rng输出进行随机切分
            # Model forward
            # model_inputs = shard(local_host_model_inputs)
            state, train_metric = train_step(state, local_host_model_inputs, dropout_rngs)
            # state, train_metric = p_train_step(state, model_inputs, dropout_rngs) # 修改7 —— 添加dropout_rng参数
            dropout_rngs = new_dropout_rng # 修改8 —— 更新dropout_rng
            ...
            if cur_step % training_args.eval_steps == 0 and cur_step > 0:
                ...
                for i, batch_idx in enumerate(tqdm(eval_batch_idx, desc="Evaluating ...", position=2)):
                    ...
                    # )
                    metrics = eval_step(state.params, model_inputs.data)
